...
class MazeGame{
public:
	Maze* createMaze(MazeFactory* aFactroy){
		Maze* maze;
		Room* r1, * r2;
		Door* d;
		maze = aFactroy->makeMaze();
		r1 = aFactroy->makeRoom();
		r2 = aFactroy->makeRoom();
		d = aFactroy->makeDoorFrom(r1, r2);
		maze->addRoom(r1);
		maze->addRoom(r2);
		r1->setAdjRoom(NORTH, aFactroy->makeWall());
		r1->setAdjRoom(SOUTH, aFactroy->makeWall());
		r1->setAdjRoom(WEST, aFactroy->makeWall());
		r1->setAdjRoom(EAST, d);
		r2->setAdjRoom(NORTH, aFactroy->makeWall());
		r2->setAdjRoom(SOUTH, aFactroy->makeWall());
		r2->setAdjRoom(EAST, aFactroy->makeWall());
...
{
	Maze::initRand();
	MazeGame* g1 = new MazeGame();
	// MazeFactory* f1 = new MazeFactory();
	MagicMazeFactory* f1 = new MagicMazeFactory();
	Maze* m1 = g1->createMaze(f1);
	m1->print(0);
	m1->print(1);
